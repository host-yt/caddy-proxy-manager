...
- **Later upgrades roll normally.** From this release on, replicas heartbeat their session generation into Redis and `/readyz` fences on it in one direction only: the newest generation present serves, older replicas go unready and drain, and a replica that cannot publish its own heartbeat refuses traffic. Nothing extra to do for a rolling deploy. Rolling *back* to an older generation still needs every newer replica stopped first (~20s for its fleet key to expire) - the older binary is the lenient one and will not serve alongside a newer one.
