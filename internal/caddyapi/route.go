...
func BuildRoute(r Route) map[string]any {
	...
	if r.SSOProviderURL != "" {
		...
		// proxies to https://sso.yt, the default rewrite changes Host to
		// "sso.yt" → no Provider matches → IdP returns a 404 page.
		...
		// fetch, so if Authentik replies with a redirect to sso.yt the
