...
// ContactPage renders GET /app/contact.
func (h *ClientHandlers) ContactPage(w http.ResponseWriter, r *http.Request) {
	d := contactData{baseAppData: h.base(r, "Contact support")}
	d.MailerReady = h.Mailer != nil
	h.render(w, "contact", d)
}

// ContactSubmit handles POST /app/contact; sends message to the super-admin.
func (h *ClientHandlers) ContactSubmit(w http.ResponseWriter, r *http.Request) {
	sess := middleware.SessionFromContext(r.Context())
	db := h.DB()
	if h.Mailer == nil {
		redirectWithFlash(w, r, "/app/contact", "", "Email not configured")
		return
	}
	...
	if subject == "" || message == "" {
		redirectWithFlash(w, r, "/app/contact", "", "subject and message required")
		return
	}
	ctx, cancel := context.WithTimeout(r.Context(), 5*time.Second)
	defer cancel()
	// Rate-limit: max 5 contact messages per user per day.
	if db != nil && sess != nil {
		var count int
		_ = db.QueryRowContext(ctx,
			`SELECT COUNT(*) FROM audit_log
			 WHERE user_id=? AND action="client.contact.sent" AND created_at >= NOW() - INTERVAL 1 DAY`,
			sess.UserID).Scan(&count)
		if count >= 5 {
			redirectWithFlash(w, r, "/app/contact", "", "Too many messages today. Try again tomorrow.")
			return
		}
	}
	// Resolve the super-admin email to send to.
	var adminEmail string
	if db != nil {
		_ = db.QueryRowContext(ctx,
			`SELECT email FROM users WHERE role="super_admin" AND is_active=1 ORDER BY id LIMIT 1`,
		).Scan(&adminEmail)
	}
	if adminEmail == "" {
		redirectWithFlash(w, r, "/app/contact", "", "Support not available right now")
		return
	}
	...
	fullSubject := "[HPG Client] " + subject + " (from: " + fromEmail + ")"
	fullBody := message + "\n\n---\nSent from client portal by: " + fromEmail
	if err := h.Mailer.Send(ctx, adminEmail, fullSubject, "notice", map[string]any{
		"Subject": subject,
		"Body":    fullBody,
	}); err != nil {
		redirectWithFlash(w, r, "/app/contact", "", "Failed to send. Try again.")
		return
	}
	if sess != nil {
		audit.Write(ctx, db, h.Logger, r, audit.Entry{
			UserID: &sess.UserID, Action: "client.contact.sent",
			Entity: "contact", Meta: map[string]any{"subject": subject},
		})
	}
